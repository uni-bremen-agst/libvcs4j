...
public class Seed {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final var cpd = new CPDRunner(100);
        // Associates violations with previous violations
        final var pmdMapping = new Mapping<String>();
        final var cpdMapping = new Mapping<String>();
        // Tracks lifespan IDs for violations
        final var violationLifespans = new IdentityHashMap<Mappable<String>, UUID>();
        // Tracks changes for violations
        final var violationChanges = new IdentityHashMap<Mappable<String>, Integer>();
        ...
        for (final var revisionRange : engine) {
            ...
            try {
                ...
                final var mappingResult = pmdMapping.map(violations, revisionRange);

                final var jsonCodeSmells = new ArrayList<JSONObject>();

                System.out.println("PMD Analysis");
                for (final var violation : violations) {
                    ...
                    if (predecessor.isPresent()) {
                        ...
                        //Check if current violation is different from predecessor
                        changes = violationChanges.get(predecessor.get());
                        var chOriginalRangeList = violation.getRanges();
                        var chPredecessorRangeList = predecessor.get().getRanges();
                        var contentsChanged = false;
                        if (chOriginalRangeList.size() != chPredecessorRangeList.size()) {
                            //List Size isn't identical, CodeSmell has changed
                            //Only relevant for CPD Duplications as PMD Violations only have one range per Violation
                            contentsChanged = true;
                        } else {
                            //Compare contents of both ranges.
                            final var predecessorRangeContent = new ArrayList<>();
                            for (final var chRange : chPredecessorRangeList) {
                                predecessorRangeContent.add(chRange.readContent());
                            }

                            final var originalRangeContent = new ArrayList<>();
                            for (final var chRange : chOriginalRangeList) {
                                originalRangeContent.add(chRange.readContent());
                            }

                            for (var fileContent : predecessorRangeContent) {
                                final var index = originalRangeContent.indexOf(fileContent);
                                if (index < 0) {
                                    contentsChanged = true;
                                } else {
                                    originalRangeContent.remove(index);
                                }
                            }
                        }
                        if (contentsChanged) {
                    ...
                    violationChanges.put(violation, changes);
                    ...
                    final var jsonCodeSmell = new JSONObject(
                        Map.ofEntries(
                            ...
                            Map.entry("changes", changes)
                        )
                    );
                    jsonCodeSmells.add(jsonCodeSmell);
                }
                //CPD Loop
                System.out.println("CPD Analysis");

                final var cpdViolations = cpd.analyze(revision).getViolations();
                final var cpdMappingResult = cpdMapping.map(cpdViolations, revisionRange);

                for (final var violation : cpdViolations) {
                    var lifespanID = UUID.randomUUID();
                    var changes = 0;
                    final var predecessor = cpdMappingResult.getPredecessor(violation);
                    if (predecessor.isPresent()) {
                        lifespanID = violationLifespans.get(predecessor.get());
                        if (lifespanID == null) {
                            throw new RuntimeException("Missing lifespan id for predecessor finding");
                    ...
                    final var jsonCodeSmell = new JSONObject(
                        Map.ofEntries(
                            Map.entry("kind", violation.getMetadata().get()),
                            Map.entry("lifespan", lifespanID.toString()),
                            Map.entry("ordinal", cpdMappingResult.getOrdinal()),
                            Map.entry("locations", jsonLocations),
                            Map.entry("changes", changes)
                        )
                    );
