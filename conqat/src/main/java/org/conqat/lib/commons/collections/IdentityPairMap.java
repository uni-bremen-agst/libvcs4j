/*-------------------------------------------------------------------------+
|                                                                          |
| Copyright 2005-2011 The ConQAT Project                                   |
|                                                                          |
| Licensed under the Apache License, Version 2.0 (the "License");          |
| you may not use this file except in compliance with the License.         |
| You may obtain a copy of the License at                                  |
|                                                                          |
|    http://www.apache.org/licenses/LICENSE-2.0                            |
|                                                                          |
| Unless required by applicable law or agreed to in writing, software      |
| distributed under the License is distributed on an "AS IS" BASIS,        |
| WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. |
| See the License for the specific language governing permissions and      |
| limitations under the License.                                           |
+-------------------------------------------------------------------------*/
package org.conqat.lib.commons.collections;

import java.util.IdentityHashMap;
import java.util.Map;

/**
 * A map using (unordered) pairs as key which are compared by reference.
 * 
 * @author hummelb
 * @author $Author: kinnen $
 * @version $Rev: 41751 $
 * @ConQAT.Rating GREEN Hash: 70BD41E1FE022CE0787C58AB0C6E0AE3
 */
public class IdentityPairMap<T, V> {

	/** Internal storage. */
	private final Map<T, Map<T, V>> map = new IdentityHashMap<T, Map<T, V>>();

	/** Adds the given value for the given pair. */
	public void put(ImmutablePair<T, T> pair, V value) {
		put(pair.getFirst(), pair.getSecond(), value);
	}

	/** Adds the given value for the given pair. */
	public void put(T t1, T t2, V value) {
		insert(t1, t2, value);
		insert(t2, t1, value);
	}

	/** Helper method for insertion */
	private void insert(T t1, T t2, V value) {
		Map<T, V> m = map.get(t1);
		if (m == null) {
			m = new IdentityHashMap<T, V>();
			map.put(t1, m);
		}
		m.put(t2, value);
	}

	/** Returns whether the pair is contained. */
	public boolean contains(ImmutablePair<T, T> pair) {
		return contains(pair.getFirst(), pair.getSecond());
	}

	/** Returns whether the pair (t1, t2) is contained. */
	public boolean contains(T t1, T t2) {
		Map<T, V> m = map.get(t1);
		if (m == null) {
			return false;
		}
		return m.containsKey(t2);
	}

	/**
	 * Returns the element stored at the pair or <code>null</code> if not
	 * stored.
	 */
	public V get(ImmutablePair<T, T> pair) {
		return get(pair.getFirst(), pair.getSecond());
	}

	/**
	 * Returns the element stored at pair (t1, t2) or <code>null</code> if not
	 * stored.
	 */
	public V get(T t1, T t2) {
		Map<T, V> m = map.get(t1);
		if (m == null) {
			return null;
		}
		return m.get(t2);
	}

	/** Clears the map. */
	public void clear() {
		map.clear();
	}
}